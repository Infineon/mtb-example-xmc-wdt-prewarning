...
A pre-warning alarm is triggered before a watchdog reset only if the watchdog timer is not serviced properly. When a prewarning condition occurs, the User LED1 blinks once. After a watchdog reset, the MCU checks the reason for the last reset; if it was caused by a watchdog reset, the User LED2 blinks at a faster rate.
...
3. Confirm the operation of the example:
   ...
   1. Confirm that USER_LED1 blinks 5 times means that the watchdog timer is serviced properly by the system tick at every 1 second.

   2. Confirm that USER_LED2 is blinking once when the prewarning interrupt is triggered before watchdog reset due to the non-servicing of the watchdog timer.

   3. Confirm that USER_LED1 blinks rapidly after a watchdog timer reset.
...
Device Configurator uses the default configuration for both XMC1400 Boot Kit and XMC4700 Relax Kit. The window lower-bound register (WLB) and the window upper-bound register (WUB) are configured for servicing the window boundary. The prewarning mode is enabled by writing to the WDT control register (CTR).

In the XMC1400 Boot Kit, the prewarning alarm is routed to `IRQ1_Handler`, while in the XMC4700 Relax kit, it is routed to `NMI_Handler`.
...
Document Title: *CE232800* - *XMC MCU: WDT Prewarning*
